...
def execute_outputs(output_info, log, regex, document_data, database):
    ...
    if output_info['output_type_id'] == 'export_xml':
        ...
        verifier_exports.export_coog(output_info['data'], document_data, log, database)
...
def process(args, file, log, config, files, ocr, regex, database, docservers, configurations, languages):
    ...
    if ai_llm:
        ...
        if llm_model:
            ai_model_name = llm_model[0]['name']
            log.info('Using AI LLM to find document details : ' + ai_model_name)
        else:
            ...
            for value in ai_invoice_values:
                if ai_invoice_values[value] and value not in datas['datas']:
                    if isinstance(ai_invoice_values[value], (str, int, float)):
                        log.info('Value found using AI : ' + value + ' = ' + str(ai_invoice_values[value]))
