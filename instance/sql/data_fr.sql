...
INSERT INTO "configurations" ("label", "data", "display") VALUES ('loginBottomMessage', '{"type": "string", "value": "<p><strong><a href=\"https://edissyum.gitbook.io/open-capture/utilisation/introduction\" target=\"_blank\" rel=\"noopener\"><span style=\"color: rgb(151, 191, 61);\">Acc&eacute;der au guide d''utilisation d''Open-Capture</span></a></strong></p>", "description": "Court message affiché sur le bas de l''écran d''accueil"}', false);
...
INSERT INTO "form_models_field" ("id", "form_id", "fields") VALUES (5, 5, '{"batch_metadata": [], "document_metadata": []}');
ALTER SEQUENCE "form_models_id_seq" RESTART WITH 6;
ALTER SEQUENCE "form_models_field_id_seq" RESTART WITH 6;
...
INSERT INTO "workflows" ("id", "workflow_id", "label", "module", "input", "process", "output") VALUES (1, 'default_workflow', 'Workflow par défaut', 'verifier', '{
    ...
    "script": "# La fonction `main` sera appelé après l''éxécution des chaînes sortantes, ne changez pas le nom de cette fonction\n# Ce script sera effectué après la partie `sortie`\n\n# Depuis l''étape `sortie` vous avez accès aux variables suivantes :\n#   - custom_id --> (string) Identifiant du custom\n#   - document_id --> (integer) Identifiant du document en BDD\n#   - file --> (string) Chemin complet du fichier sur le docserver\n#   - opencapture_path --> (string) Racine du dossier d''installation Open-Capture\n#   - outputs --> (array) Liste des paramètres de chaînes sortantes associées au workflow\n#   - datas --> (array) Informations du document après traitement depuis l''interface (contient datas, positions et pages)\n#   - log --> (classe Log) Instance de la classe Log permettant de logger (info ou error) différentes données\n\n# Depuis l''étape `sortie` vous avez accès aux fonctions suivantes :\n#   - update_document_data() --> Permet la modification des données du document\n#       - args : (array) Tableau associatif contenant les données à modifier :\n#           - Liste des champs récupérable :\n#               - Fournisseur :\n#                   - name, address1, address2, postal_code, city, country, email, vat_number, iban, siret, siren\n#               - Facturation :\n#                   - invoice_number, document_date, document_due_date, quotation_number, delivery_number, vat_amount\n#                   - no_rate_amount, vat_rate, total_ht, total_vat, total_ttc\n#               - Champ personnalisés : identifiant du champ personnalisé\n\n#           - Exemple de l''appel (custom_id et document_id sont obligatoires) :\n#               - update_document_data({\n#                   ''custom_id: args[''custom_id''],\n#                   ''document_id'': args[''document_id''],\n#                   ''datas'': {\n#                       ''invoice_number'': args[''datas''][''invoice_number''] + ''-EDISSYUM''\n#                   }\n#                 })\n\n# La liste des librairies Python disponible sont visibles ici :\n# https://github.com/edissyum/opencapture/blob/master/install/pip-requirements.txt\n\n# Ce script est un exemple permettant de mettre un numéro de facture par défaut\n# Si aucune numéro de facture n''a été trouvé, afin de faciliter la recherche\n\nimport xml.etree.ElementTree as Et\nfrom src.backend.scripting_functions import update_document_data\n\n\ndef main(args):\n    return ''DISABLED'' # REMOVE THIS LINE TO ACTIVATE SCRIPTING\n    args[''log''].info(f\"[OUTPUT SCRIPT] Traitement des données du fichier {args[''file'']} après traitement\")\n\n    if ''document_id'' in args and args[''document_id''] is not None:\n        args[''log''].info(f\"[OUTPUT SCRIPT] Identifiant du document : {args[''document_id'']}\")\n\n        if ''invoice_number'' in args[''datas'']:\n            args[''log''].info(''[OUTPUT SCRIPT] Ajout d\\''un suffixe sur le numéro de facture'')\n            update_document_data({\n                ''custom_id'': args[''custom_id''],\n                ''document_id'': args[''document_id''],\n                ''data'': {\n                    ''invoice_number'': str(args[''datas''][''invoice_number'']) + ''-EDISSYUM''\n                }\n            })\n\n    if ''outputs'' in args and args[''outputs'']:\n        for output in args[''outputs'']:\n            if output[''output_type_id''] == ''export_xml'':\n                args[''log''].info(''[OUTPUT SCRIPT] Modification du fichier XML'')\n                xml_content = Et.parse(output[''file_path''])\n                print(output[''file_path''])\n                root = xml_content.getroot()\n                for data in root.findall(''DATAS''):\n                    data.find(''invoice_number'').text = data.find(''invoice_number'').text + ''-EDISSYUM''\n                xml_content.write(output[''file_path''])\n    return True\n",
    "outputs_id": [
        1,
        3
    ]
}');
INSERT INTO "workflows" ("id", "workflow_id", "label", "module", "input", "process", "output") VALUES (2, 'ocr_only', 'OCRisation simple par défaut', 'verifier', '{"ai_model_id": null, "customer_id": null, "facturx_only": false, "input_folder": "/var/share/entrant/verifier/ocr_only/", "apply_process": false, "rotation": "no_rotation", "remove_blank_pages": true, "splitter_method_id": "no_sep", "separate_by_document_number_value": 2}', '{"ai_llm": "no_ai_llm", "form_id": null, "system_fields": [], "custom_fields": [], "use_interface": false, "delete_documents": false, "override_supplier_form": false, "allow_automatic_validation": false}', '{"outputs_id": [3]}');
INSERT INTO "workflows" ("id", "workflow_id", "label", "module", "input", "process", "output") VALUES (3, 'facturx', 'Workflow FacturX par défaut', 'verifier', '{"ai_model_id": null, "customer_id": null, "facturx_only": true, "input_folder": "/var/share/entrant/verifier/facturx/", "apply_process": true, "rotation": "no_rotation", "remove_blank_pages": true, "splitter_method_id": "no_sep", "separate_by_document_number_value": 2}', '{"ai_llm": "no_ai_llm", "form_id": 2, "system_fields": ["name", "invoice_number", "quotation_number", "document_date", "document_due_date", "footer", "facturx"], "custom_fields": [], "use_interface": true, "convert_function": "pdf2image", "tesseract_function": "line_box_builder", "delete_documents": false, "override_supplier_form": false, "allow_automatic_validation": false}', '{"outputs_id": []}');
INSERT INTO "workflows" ("id", "workflow_id", "label", "module", "input", "process", "output") VALUES (4, 'default_workflow', 'Workflow par défaut', 'splitter', '{"ai_model_id": 0, "customer_id": 1, "input_folder": "/var/share/entrant/splitter/", "apply_process": true, "remove_blank_pages": false, "splitter_method_id": "qr_code_OC", "script": "# La fonction `main` sera appelé avant le traitement d''un document, ne changez pas le nom de cette fonction\n# Ce script sera effectué avant la partie `traitement`, si cette dernière est activée\n\n# Depuis l''étape `entrée` vous avez accès aux variables suivantes :\n#   - ip --> (string) Adresse IP de l''utilisateur\n#   - user_info --> (string) Informations sur l''utilisateur courant\n#   - custom_id --> (string) Identifiant du custom\n#   - file --> (string) Chemin complet du fichier à traiter\n#   - opencapture_path --> (string) Racine du dossier d''installation Open-Capture\n#   - log --> (classe Log) Instance de la classe Log permettant de logger (info ou error) différentes données\n#   - customer_id --> (integer) Identifiant du compte client\n\n# La liste des librairies Python disponible sont visibles ici :\n# https://github.com/edissyum/opencapture/blob/master/install/pip-requirements.txt\n\ndef main(args):\n    args[''log''].info(\"Input scripting...\")"}', '{"form_id": 5, "rotation": "no_rotation", "use_interface": true, "delete_documents": false, "allow_automatic_validation": false, "script": "# La fonction `main` sera appelé avant le traitement d''un document, ne changez pas le nom de cette fonction\n# Ce script sera effectué avant la partie `Sortie`, si cette dernière est activée\n\n# Depuis l''étape `entrée` vous avez accès aux variables suivantes :\n#   - custom_id --> (string) Identifiant du custom\n#   - batches_id -> (list) Identifiants des lots injectés \n#   - file --> (string) Chemin complet du fichier à traiter\n#   - opencapture_path --> (string) Racine du dossier d''installation Open-Capture\n#   - log --> (classe Log) Instance de la classe Log permettant de logger (info ou error) différentes données\n#   - customer_id --> (integer) Identifiant du compte client\n\n# Depuis l''étape `traitement` vous avez accès aux fonctions suivantes :\n#   - update_document_data() --> Permet la modification des données du document\n#       - args : (array) Tableau associatif contenant les données à modifier :\n#           - Exemple de l''appel (custom_id et batches_id sont obligatoires) :\n#               - update_document_data({\n#                   ''custom_id: args[''custom_id''],\n#                   ''batches_id'': args[''batches_id''],\n#                   ''datas'': {\n#                       ''custom_1'': ''value''\n#                   }\n#                 })\n#   - execute_output_splitter() --> Permet l''execution de chaîne sortante. Annule les chaines sortantes associées au formulaire ou au workflow\n#\t\t- args : (array)\n#\t\t\t- Exemple de l''appel (le args[''outputs''] est obligatoire et contient les identifiant des chaines sortantes à executer)\n#\t\t\t\t- args[''outputs''] = [1, 2]\n#        \t\t  execute_output_splitter(args)\n#\t\t\t\t  return ''stop_workflow''\n\n# La liste des librairies Python disponible sont visibles ici :\n# https://github.com/edissyum/opencapture/blob/master/install/pip-requirements.txt\n\ndef main(args):\n    args[''log''].info(\"Process scripting...\")\n"}', '{"outputs_id": [], "script": "# La fonction `main` sera appelé avant le traitement d''un document, ne changez pas le nom de cette fonction\n# Ce script sera effectué avant la partie `traitement`, si cette dernière est activée\n\n# Depuis l''étape `entrée` vous avez accès aux variables suivantes :\n#   - batch_id -> (int) Identifiant du lot validé\n#   - custom_id --> (string) Identifiant du custom \n#   - file --> (string) Chemin complet du fichier à traiter\n#   - opencapture_path --> (string) Racine du dossier d''installation Open-Capture\n#   - log --> (classe Log) Instance de la classe Log permettant de logger (info ou error) différentes données\n#   - customer_id --> (integer) Identifiant du compte client\n\n# La liste des librairies Python disponible sont visibles ici :\n# https://github.com/edissyum/opencapture/blob/master/install/pip-requirements.txt\n\ndef main(args):\n    args[''log''].info(\"Output scripting...\")\n"}');
INSERT INTO "workflows" ("id", "workflow_id", "label", "module", "input", "process", "output") VALUES (5, 'default_mail', 'Workflow courrier', 'verifier', '{"script": null, "rotation": "no_rotation", "ai_model_id": 0, "customer_id": null, "facturx_only": null, "input_folder": "/var/share/entrant/verifier/mail/", "apply_process": true, "remove_blank_pages": null, "splitter_method_id": "no_sep", "separate_by_document_number_value": 2}', '{"script": null, "form_id": 3, "api_only": null, "custom_fields": null, "system_fields": ["contact", "subject", "document_date"], "use_interface": true, "convert_function": "pdf2image", "delete_documents": null, "tesseract_function": "line_box_builder", "override_supplier_form": null, "allow_automatic_validation": null, "allow_third_party_validation": null}', '{"script": null, "outputs_id": [3, 1]}');
ALTER SEQUENCE "workflows_id_seq" RESTART WITH 6;
