...
INSERT INTO "regex" ("regex_id", "lang", "label", "content") VALUES ('vatAmountRegex', 'fra', 'Montant TVA', '((MONTANT|TOTAL|DONT)\s*TVA(\s*[0-9.,]*\s*%)?(\s*\(SUR\s*FACTURE\s*\))?|TVA\s*[0-9.,]*\s*%|TVA\s*[0-9.,]*\s*%?\s*SUR\s*[0-9.,]*\s*[A-Z\s'']*|%\s*TVA)\s*(€|EUROS|EUR)?.*');
