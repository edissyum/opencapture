import { Browser, ChildProperty, Collection, Complex, Component, Event, EventHandler, Internationalization, NotifyPropertyChanges, Property, Touch, createElement, extend, isNullOrUndefined, merge, print, remove, select } from '@syncfusion/ej2-base';
...
function rotateTextSize(font, text, angle) {
    ...
    const htmlObject = renderer.createText(options, text);
...
class Axis extends ChildProperty {
    ...
    multiLevelLabelSize(innerPadding, index) {
        ...
        const size = (this.orientation === 'Horizontal') ? this.xAxisMultiLabelHeight[index] : this.yAxisMultiLabelHeight[index];
    ...
    getMaxLabelSize(axis, heatmap) {
        ...
        let count = 1;
        const row = 1;
        const interval = (axis.valueType === 'DateTime' && axis.showLabelOn !== 'None') ?
    ...
    calculateCategoryAxisLabels() {
        ...
        if (labels && labels.length > 0) {
            for (let i = min; i <= max; i = i + interval) {
                const value = labels[i] ? labels[i].toString() : i.toString();
                this.axisLabels.push(value);
            }
        }
        else {
...
class AxisHelper {
    ...
    drawXAxisLabels(axis, parent, rect) {
        ...
        for (let i = 0, len = labels.length; i < len; i++) {
            ...
            if (axis.labelIntersectAction === 'MultipleRows' && axis.labelRotation === 0) {
                const a = axis.opposedPosition ? -(axis.multipleRow[i].index - 1) : (axis.multipleRow[i].index - 1);
            ...
            if (angle % 360 !== 0) {
                ...
                const rotateSize = rotateTextSize(axis.textStyle, label, angle);
            ...
            const options = new TextOption(heatMap.element.id + '_XAxis_Label' + i, new TextBasic(x, y, (angle % 360 === 0) ? 'start' : 'middle', label, angle, transform), axis.textStyle, axis.textStyle.color || heatMap.themeStyle.axisLabel);
    ...
    drawYAxisLabels(axis, parent, rect) {
        ...
        const anchor = axis.opposedPosition ? 'start' : 'end';
        padding = axis.opposedPosition ? padding : -padding;
        let labelElement;
        let borderElement;
        if (!heatMap.enableCanvasRendering) {
        ...
        for (let i = 0, len = labels.length; i < len; i++) {
            ...
            const position = labelRect.height / 2; //titlePositionY(lableRect, 0, 0, axis.textStyle);
            const axisWidth = this.heatMap.cellSettings.border.width >= 20 ? (this.heatMap.cellSettings.border.width / 2) : 0;
            const x = labelRect.x + padding + (axis.opposedPosition ? axisWidth : -axisWidth);
            ...
            const y = (labelRect.y - indexValue) + (axis.isInversed ? position : -position);
            label = axis.enableTrim ? textTrim(axis.maxLabelLength, labels[i], axis.textStyle) : labels[i];
            const options = new TextOption(heatMap.element.id + '_YAxis_Label' + i, new TextBasic(x, y, anchor, label, 0, 'rotate(' + 0 + ',' + (x) + ',' + (y) + ')', 'middle'), axis.textStyle, axis.textStyle.color || heatMap.themeStyle.axisLabel);
            ...
            if (label.indexOf('...') !== -1) {
                const xValue = axis.opposedPosition ? x : (x - elementSize.width);
    ...
    drawXAxisBorder(axis, parent, rect, lableX, width, index) {
        ...
        const endY1 = axis.isInversed ? (lableX + width + padding) : (lableX - padding);
        const endY2 = axis.isInversed ? (lableX - padding) : (lableX + width + padding);
...
class Series {
    ...
    renderRectSeries() {
        ...
        let tempVal = 0;
        const themeStyle = heatMap.themeStyle;
...
class Legend {
    ...
    renderLegendItems() {
        ...
        if (heatMap.paletteSettings.type === 'Fixed') {
            ...
            for (let i = 0; i < colorCollection.length; i++) {
                const visibility = !isNullOrUndefined(this.visibilityCollections[i]) ? this.visibilityCollections[i] : true;
    ...
    renderLegendTitleTooltip(e, pageX, pageY) {
        if (e.target.id.indexOf('_legendTitle') !== -1 && e.target.textContent.indexOf('...') > -1) {
            ...
            const element = document.getElementById(this.heatMap.element.id + '_legendTitle_Tooltip');
            if (element) {
    ...
    renderLegendLabelTooltip(e, pageX, pageY) {
        if (e.target.id.indexOf('_Legend_Label') !== -1 && e.target.textContent.indexOf('...') > -1) {
            ...
            const element = document.getElementById(this.heatMap.element.id + '_LegendLabel_Tooltip');
            if (element) {
                element.style.visibility = 'hidden';
            }
        }
...
let HeatMap = class HeatMap extends Component {
    ...
    render() {
        ...
        if (this.theme === 'Bootstrap5' || this.theme === 'Bootstrap5Dark') {
            let textSettings = { title: { textStyle: { size: "12px", fontFamily: "system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', 'Liberation Sans', sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol', 'Noto Color Emoji'", fontWeight: "500" } }, textStyle: { size: "12px", fontFamily: "system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', 'Liberation Sans', sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol', 'Noto Color Emoji'" } };
    ...
    renderBorder() {
        ...
        const width = 0;
        const rect = new RectOption(this.element.id + '_HeatmapBorder', this.themeStyle.background, this.border, 1, new Rect(width / 2, width / 2, this.availableSize.width - width, this.availableSize.height - width));
    ...
    wireEvents() {
         ...
         */
        // eslint-disable-next-line
        const touchObj = new Touch(this.element, {
            tapHold: () => {
