...
function getTemplateFunction(template, gauge) {
    ...
    try {
        if (gauge.isBlazor) {
            const numb = template.match(/\d+/g).toString();
...
class Annotations {
    ...
    createTemplate(element, annotationIndex, axisIndex) {
        ...
        if (this.gauge.isBlazor) {
            const { cancel, name, content, textStyle } = argsData;
    ...
    updateLocation(element, axis, annotation) {
        ...
        const elementRect = measureElementRect(element);
...
class PointerRenderer {
    ...
    setPointerValue(axis, pointer, value) {
        ...
        const oldEndValue = ((((pointer.currentRadius - (pointer.pointerWidth / 2)) * ((endAngle * Math.PI) / 180) +
            (radius / minRadius)) / (pointer.currentRadius - (pointer.pointerWidth / 2))) * 180) / Math.PI;
        const roundStartAngle = ((((pointer.currentRadius) * ((startAngle * Math.PI) / 180) +
            radius) / (pointer.currentRadius)) * 180) / Math.PI;
        const roundEndAngle = ((((pointer.currentRadius) * ((endAngle * Math.PI) / 180) -
            radius) / (pointer.currentRadius)) * 180) / Math.PI;
                ...
                const axisLabelRenderSuccess = (argsData) => {
                    if (!argsData.cancel) {
                        ...
                        if (i === max && this.gauge.isBlazor && document.getElementById(this.gauge.element.id + '_AxesCollection')) {
                            const currentLast = axis.visibleLabels.length ? axis.visibleLabels[axis.visibleLabels.length - 1].value
                                : null;
        ...
        this.trigger('gaugeMouseMove', args, (observedArgs) => {
            ...
            if (!args.cancel) {
                ...
                if (this.enablePointerDrag && this.activePointer) {
                    ...
                    const dragRangeInd = parseInt(this.activeRange.pathElement[0].id.slice(-1), 10);
                    dragArgs = {
                        axis: this.activeAxis,
                        name: dragMove,
                        type: rangeMove,
