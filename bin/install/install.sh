...
export PGPASSWORD=$databasePassword && psql -U"$databaseUsername" -h"$hostname" -p"$port" -c "UPDATE docservers SET path='$customPath/bin/scripts/ai/verifier/train_data/' WHERE docserver_id = 'VERIFIER_TRAIN_PATH_FILES'" "$databaseName" >>$INFOLOG_PATH 2>>$ERRORLOG_PATH
...
export PGPASSWORD=$databasePassword && psql -U"$databaseUsername" -h"$hostname" -p"$port" -c "UPDATE docservers SET path='$customPath/bin/scripts/ai/verifier/models/' WHERE docserver_id = 'VERIFIER_AI_MODEL_PATH'" "$databaseName" >>$INFOLOG_PATH 2>>$ERRORLOG_PATH
